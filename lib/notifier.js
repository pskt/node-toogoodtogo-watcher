...
const { notifyPushAlert } = require("./pushalert");
...
function hasListeners$() {
  return combineLatest(
    ...
    of(config.get("notifications.pushAlert.enabled")),
...
function notifyIfChanged(businesses) {
  ...
  const url = createUrl(filteredBusinesses);
  ...
  if (filteredBusinesses.length > 0) {
    ...
    if (config.get("notifications.pushAlert.enabled")) {
      notifyPushAlert(textMessage, htmlMessage, url);
    }
